...
export class FilterUnitsService {
  ...
  filter(results: Locations[], showClose:boolean, hour: string) {
    ...
    if(hour){
      const OPEN_HOUR = HOURS[hour as HOUR_INDEXES].first
      const CLOSE_HOUR = HOURS[hour as HOUR_INDEXES].last
